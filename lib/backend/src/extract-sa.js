...
module.exports = {
    extract: function(req, res, _process, util, create, path, zcache, counter) {
        util.log("sadf type determined: " + zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["sadf_type_det"]);
        var commandObj,
            txtCmd = ['LC_ALL=C', 'scripts/vos/analysis/bin/sadf', '-x'];
        txtCmd[1] = txtCmd[1] + '-' + zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["sadf_type_det"] + '-' + '64';

        var conv_path = zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["sa_file_path_conv"];
        if(conv_path){
          txtCmd.push(conv_path);
        } else {
          txtCmd.push(zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["sa_file_path"]);
        }
        txtCmd.push('--');
        var sar_file = path.join(zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["datafile_path"], "sar.xml");
        ...
        commandObj.stdout.on('end', function () {
            ...
            var sar_cmd = function() {
                ...
                nodenameObj.stdout.on('data', function (data) {
                    zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["nodename"] += data.toString();
                });
                nodenameObj.stdout.on('end', function(){
                    var ix_res = {};
                    var sarObj, sarCmd = ['scripts/vos/analysis/bin/index-sar',
                                          sar_file,
                                          zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["nodename"]];
                    ...
                    sarObj.stdout.on('end', function(){
                        ...
                        create.dashboard(_process, util, zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["nodename"],
                    ...
                    sarObj.stdout.on('close', function(){
                        util.log('[ES data ingested] -- ' + zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["nodename"]);
                        if (!!zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["nodename"]) {
                            ...
                            callback(function(){
                              res.write("Success! <u>Nodename: <b>" +
                                        zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["nodename"] + "</b></u>");
        ...
        commandObj.on('close', function (code) {
            zcache["session_data"][req.sessionID]["file_ops_blob"][counter]["sadf_type_det"] = '';
