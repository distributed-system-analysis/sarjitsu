...
class SysStatParse(object):
    ...
    def _error(self, msg, *args):
        ...
        if _DEBUG > 8:
            import pdb; pdb.set_trace()
            app.logger.error(repr(self.stack))

    def _warn(self, msg, *args):
        app.logger.warn(msg % args)
        if _DEBUG > 9:
            import pdb; pdb.set_trace()
            app.logger.warn(repr(self.stack))

    def _dump_actions(self):
        for act in self.actions:
            app.logger.info(json.dumps(act, indent=4, sort_keys=True))
    ...
    def _push_dict(self, obj, name=None):
        ...
        if _DEBUG > 8:
            app.logger.info(name, "pushed", self.curr_element_dict, self.curr_element_dict_stack)

    def _pop_dict(self, name=None):
        if _DEBUG > 8:
            if len(self.curr_element_dict_stack) == 0:
                import pdb; pdb.set_trace()
            app.logger.info(name, "popping", self.curr_element_dict, self.curr_element_dict_stack)
        else:
    ...
    def _bulk_upload(self):
        ...
        if len(self.actions) == 0:
            app.logger.error('0 actions found..')

        beg, end = time.time(), None
        start = beg
        if _DEBUG > 0:
            app.logger.info("\tbulk index (beg ts: %s) ..." % tstos(beg))
        delay = _read_timeout
        tries = 20
        try:
            while True:
                try:
                    ...
                    if isinstance(err.info, ul_excs.ReadTimeoutError):
                        tries -= 1
                        if tries > 0:
                            app.logger.warn("\t\tWARNING (end ts: %s, duration: %.2fs):"
                            ...
                            delay *= 2
                            beg, end = time.time(), None
                            app.logger.warn("\t\tWARNING (beg ts: %s): retrying..." %
                                    (tstos(beg)))
                            continue
                    if _DEBUG > 8:
                        import pdb; pdb.set_trace()
                    app.logger.error("\tERROR (end ts: %s, duration: %.2fs): %s" %
                    ...
                    for idx, ires in enumerate(res[1]):
                        ...
                        if sts not in (200, 201):
                            if _op_type == 'create' and sts == 409:
                                ...
                                app.logger.error("\t\tERRORS (%d of %d): %r" %
                    ...
                    if _DEBUG > 0 or lcl_errors > 0:
                        app.logger.info("\tdone (end ts: %s, duration: %.2fs,"
        ...
        if self.state == 'int-global':
            ...
            if name == 'host':
                ...
                try:
                    ...
                    app.logger.error("Seems to be an invalid sar XML file,"
        ...
        if self.curr_element is not None:
            if self.curr_element != name:
                if data_buf != '':
                    ...
                    pass
                app.logger.error('curr_element != name')
            if data_buf:
                if self.curr_element_dict is None:
                    import pdb; pdb.set_trace()
            ...
            if self.curr_meta_element != name:
                if data_buf != '':
                    ...
                    # Quietly swallow other empty elements that don't match this one
                    pass
                app.logger.error('curr_meta_element != name')
            if data_buf:
...
def call_indexer(file_path=None, _nodename=None, TS_ALL=TS_ALL,
    ...
    if cfg_name is None:
        app.logger.error("cfg_name was not supplied. Reading path from VOS_CONFIG_PATH")
    ...
    if _DEBUG > 0:
        app.logger.info("parsing %s..." % (fname if fname != '-' else '<stdin>',))
    ...
    if _DEBUG > 0:
        app.logger.info("grafana_range_begin %s" % (TS_ALL[0]))
        app.logger.info("grafana_range_end %s" % (TS_ALL[-1]))
        app.logger.info("...parsed %s (%.03f secs)" %
